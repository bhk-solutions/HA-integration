Protocol (transparent Zigbee)

- Gateway discovery stays the same (DISCOVER_GATEWAY broadcast on UDP port 50000).
- Pairing stays the same (open_join command).

Gateway -> UDP (broadcast, port 50002)

1) Device announce (sent by device over Zigbee, forwarded by gateway)
{
  "type": "device_join",
...
2) Device report (sent by device over Zigbee report cluster)
{
  "type": "device_report",
  "device_id": "A1B2C3D4E5F6",
  "payload": "2_ON",
...
HA -> Gateway (UDP, port 50000)

1) Device command (forwarded to device over command cluster)
{
  "type": "device_cmd",
  "dest": "A1B2C3D4E5F6",
  "com": "2_ON"
...
Notes:
- device_id is a hex MAC generated by the device (not IEEE).
- device_type is provided by the device announce message and is used by HA to select the handler.
